...
void bintree<T>::swap(bintree<T> & tree) {
  ...
  int aux2;
  //intercambiamos las raices
  aux = laraiz;
  laraiz = tree.laraiz;
  tree.laraiz = aux;
  //intercambiamos el tamaño
  aux2 = num_nodos;
  num_nodos = tree.num_nodos;
  tree.num_nodos = aux2;
}
...
bintree<T>::inorder_iterator::operator=(const bintree<T>::inorder_iterator & i)
{
  elnodo = i.elnodo;
  return *this;
}
...
template <typename T>
typename bintree<T>::const_inorder_iterator & bintree<T>::const_inorder_iterator::operator--() {
  if (elnodo.null())
    return *this;
